...
export class UsersService {
    ...
    async createConfirmMail(email: string, token: string) {
        transport.sendMail({
            ...
            html: `Un compte a été créé sur le reseau avec votre adresse mail, si c'est bien vous qui l'avez créé, clickez sur le lien <a href="${process.env.API_URL}/auth/confirmToken/me/token?token=${token}">ICI</a>` 
        })
    }
